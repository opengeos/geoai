...
- Dependency installer shows `Failed to install geoai-py: SSL certificate error`: update the GeoAI plugin to v1.4.4 or newer, click **Reinstall Dependencies**, and restart QGIS. Recent plugin versions retry package installs with native TLS plus trusted package hosts for PyPI, PyTorch, and uv certificate errors. If the error persists on a managed network, confirm that QGIS can access `pypi.org`, `files.pythonhosted.org`, and `download.pytorch.org`, and configure the QGIS proxy settings before reinstalling.
