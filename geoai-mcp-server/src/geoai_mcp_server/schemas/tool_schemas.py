...
class SegmentationResult(BaseResult):
    ...
    classes_found: List[str] = Field(default_factory=list, description="List of object classes found")
    coverage_percent: Optional[float] = Field(None, description="Percentage of image covered by segmentation")
...
class ClassifyLandCoverInput(BaseModel):
    ...
    num_classes: int = Field(
        default=10,
        description="Number of land cover classes.",
        ge=2,
        le=100
    )
    output_format: OutputFormat = Field(
        default=OutputFormat.GEOTIFF,
        description="Output format for classified image."
    )
    output_filename: Optional[str] = Field(
        None,
        description="Custom output filename."
...
class DetectionResult(BaseResult):
    ...
    detections_by_class: Dict[str, int] = Field(
        ...
        description="Number of detections per class"
    )
...
class ChangeDetectionInput(BaseModel):
    ...
    image1_path: str = Field(
        ...,
        description="Path to the first (earlier) image."
    )
    image2_path: str = Field(
        ...,
        description="Path to the second (later) image."
    )
    ...
    output_format: OutputFormat = Field(
        default=OutputFormat.GEOTIFF,
        description="Output format for change map."
    )
    include_statistics: bool = Field(
        default=True,
        description="Whether to include detailed change statistics."
    )
    output_filename: Optional[str] = Field(
        None,
        description="Custom output filename."
...
class ChangeDetectionResult(BaseResult):
    ...
    change_percentage: float = Field(0, description="Percentage of area with detected changes")
    change_area_sq_meters: Optional[float] = Field(None, description="Area of change in square meters")
    change_types: Dict[str, float] = Field(
        default_factory=dict,
        description="Types of changes detected and their areas"
    )
...
class DownloadImageryInput(BaseModel):
    ...
    output_subdir: Optional[str] = Field(
        None,
        description="Subdirectory within output folder to save downloads."
    )
...
class PrepareTrainingDataInput(BaseModel):
    ...
    labels_dir: str = Field(
        ...,
        description="Directory containing label masks."
    )
    tile_size: int = Field(
        default=256,
        description="Size of output tiles in pixels.",
        ge=64,
        le=2048
    )
    overlap: int = Field(
    ...
    output_subdir: Optional[str] = Field(
        None,
        description="Subdirectory for training data output."
    )
...
class DownloadResult(BaseResult):
    ...
    coverage_percent: Optional[float] = Field(None, description="Percentage of requested area covered")
...
class ExtractFeaturesInput(BaseModel):
    ...
    reference_point: Optional[tuple[int, int]] = Field(
        None,
        description="Reference point (x, y) for similarity map generation."
    )
    output_filename: Optional[str] = Field(
        None,
        description="Custom output filename."
...
class CanopyHeightInput(BaseModel):
    ...
    image_path: str = Field(
        ...,
        description="Path to the RGB input image."
    )
    output_format: OutputFormat = Field(
        default=OutputFormat.GEOTIFF,
        description="Output format for height map."
    )
    include_statistics: bool = Field(
        default=True,
        description="Whether to include height statistics."
    )
    output_filename: Optional[str] = Field(
        None,
        description="Custom output filename."
...
class VLMAnalysisInput(BaseModel):
    ...
    detect_target: Optional[str] = Field(
        None,
        description="Object description to detect (required for 'detect' task)."
    )
    output_filename: Optional[str] = Field(
        None,
        description="Custom output filename for detection results."
    )
...
class FeatureExtractionResult(BaseResult):
    ...
    feature_dimensions: tuple[int, ...] = Field(
        ...
        description="Shape of extracted features"
    )
...
class CanopyHeightResult(BaseResult):
    ...
    forest_coverage_percent: float = Field(0, description="Percentage of area with detected vegetation")


class VLMResult(BaseResult):
    """Result schema for VLM analysis."""
    task: str = Field("", description="Task that was performed")
    caption: Optional[str] = Field(None, description="Generated caption (for 'caption' task)")
    answer: Optional[str] = Field(None, description="Answer to query (for 'query' task)")
    detections: List[Dict[str, Any]] = Field(
        default_factory=list,
        description="Detected objects (for 'detect' task)"
    )
...
class CleanResultsInput(BaseModel):
    ...
    regularize_buildings: bool = Field(
        default=False,
        description="Whether to apply building footprint regularization."
    )
    output_filename: Optional[str] = Field(
        None,
        description="Custom output filename."
...
class CleanResultsResult(BaseResult):
    ...
    objects_removed: int = Field(0, description="Number of objects removed during cleaning")
...
class ListFilesResult(BaseResult):
    ...
    files: List[Dict[str, Any]] = Field(default_factory=list, description="List of files with metadata")
