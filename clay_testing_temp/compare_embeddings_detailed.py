...
def generate_embeddings_core_clay_individual(stack, lat, lon):
    ...
    for i, datetime_obj in enumerate(datetimes):
        ...
        pixels_single = transform(pixels_single).unsqueeze(0)  # Add batch dimension like wrapper
        ...
        datacube = {
            ...
            "time": torch.tensor(
                ...
                dtype=torch.float32,
                device=device,
            ).unsqueeze(0),  # Add batch dimension like wrapper
            "latlon": torch.tensor(
                lat_norm + lon_norm, dtype=torch.float32, device=device  # Same as wrapper
            ).unsqueeze(0),  # Add batch dimension like wrapper
...
def main():
    ...
    embeddings_core_individual = generate_embeddings_core_clay_individual(stack, lat, lon)
    ...
    print("\n=== Comparison Results ===")
    results = compare_embeddings(embeddings_core_individual, embeddings_wrapper, 
                                "Core Clay (individual)", "GeoAI wrapper")
