...
def generate_random_inputs(height=256, width=256, num_bands=4, num_images=3, seed=42):
    ...
    images = np.random.uniform(0, 10000, (num_images, height, width, num_bands)).astype(np.float32)
    ...
    dates = [base_date + datetime.timedelta(days=int(np.random.uniform(0, 365))) for _ in range(num_images)]
...
def main():
    ...
    print(f"   Wavelengths: {[f'{v:.3f}' for v in metadata['bands']['wavelength'].values()]}")
    ...
    images, bounds, dates, gsd = generate_random_inputs(height=256, width=256, num_bands=4, num_images=3, seed=42)
    ...
    if cls_best_tol and cls_best_tol <= 1e-4:
        ...
        print("   Small differences are likely due to processing order (batched vs individual)")
