...
def test_downstream_performance(embeddings, method_name):
    ...
    clf = svm.SVC()
    clf.fit(embeddings[fit_indices] + 100, labels[fit_indices])  # +100 offset like original
...
def main():
    ...
    print("This test verifies both approaches produce identical downstream task results")
    print("\nLoading data...")
    ...
    embeddings_wrapper = generate_embeddings_geoai_wrapper(stack, lat, lon, bounds, epsg_str, gsd)
    
    print("\n=== Numerical Comparison ===")
    max_diff = np.max(np.abs(embeddings_core - embeddings_wrapper))
    mse = np.mean((embeddings_core - embeddings_wrapper) ** 2)
    correlation = np.corrcoef(embeddings_core.flatten(), embeddings_wrapper.flatten())[0, 1]
    ...
    correct_core, total_core, pred_core = test_downstream_performance(embeddings_core, "Core Clay")
    correct_wrapper, total_wrapper, pred_wrapper = test_downstream_performance(embeddings_wrapper, "GeoAI Wrapper")
    ...
    if predictions_match and correct_core == correct_wrapper:
        ...
        print("   Small numerical differences are due to processing order and don't affect performance")
