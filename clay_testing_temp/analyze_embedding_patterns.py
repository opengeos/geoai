...
def analyze_embedding_patterns(embeddings):
    ...
    print(f"Number of patches per image: {num_patches} (1 CLS + {num_patches-1} spatial patches)")
    ...
    for img_idx in range(num_images):
        ...
        for patch_idx in range(1, num_patches):
            patch_diff = np.max(np.abs(core_all[img_idx, patch_idx, :] - wrapper_all[img_idx, patch_idx, :]))
    ...
    for img_idx in range(num_images):
        ...
        print(f"Image {img_idx}: CLS consistency difference = {cls_consistency_diff:.2e}")
        
        # Same for wrapper
        wrapper_cls_from_full = wrapper_all[img_idx, 0, :]
        wrapper_cls_separate = wrapper_cls[img_idx, :]
        
        wrapper_cls_consistency_diff = np.max(np.abs(wrapper_cls_from_full - wrapper_cls_separate))
        print(f"Image {img_idx}: Wrapper CLS consistency difference = {wrapper_cls_consistency_diff:.2e}")
...
def main():
    ...
    print("Analyzing differences between core Clay and wrapper across ALL patch embeddings")
    ...
    embeddings = generate_embeddings_both_methods(stack, lat, lon, bounds, epsg_str, gsd)
    ...
    if max_diff < 1e-6:
        ...
        print("   Small differences likely due to processing order, functionally equivalent")
    else:
