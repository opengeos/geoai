...
def get_data():
    ...
    size, gsd = 256, 10
    bounds = (
        coords[0] - (size * gsd) // 2, coords[1] - (size * gsd) // 2,
        coords[0] + (size * gsd) // 2, coords[1] + (size * gsd) // 2,
    )
    
    stack = stackstac.stack(
        items, bounds=bounds, snap_bounds=False, epsg=epsg, resolution=gsd,
        dtype="float64", rescale=False, fill_value=np.nan,
        assets=["blue", "green", "red", "nir"], resampling=Resampling.nearest,
    ).compute()
...
def generate_all_embeddings_core_clay(stack, lat, lon):
    ...
    model = ClayMAEModule.load_from_checkpoint(
        "~/.cache/clay/clay-v1.5.ckpt", model_size="large", 
    ...
    waves = [metadata[platform].bands.wavelength[str(band.values)] for band in stack.band]
...
def generate_all_embeddings_wrapper(stack, lat, lon, bounds, epsg_str, gsd):
    ...
    wgs84_bounds = (
        bounds_gdf.iloc[0].geometry.x, bounds_gdf.iloc[0].geometry.y,
        bounds_gdf.iloc[1].geometry.x, bounds_gdf.iloc[1].geometry.y,
    )
...
def compare_embeddings_comprehensive(emb1_all, emb1_cls, emb2_all, emb2_cls, name1="Method 1", name2="Method 2"):
    ...
    for tolerance in tolerance_levels:
        ...
        if are_identical:
            print(f"✅ All embeddings are functionally identical at tolerance {tolerance}")
    ...
    for tolerance in tolerance_levels:
        ...
        if are_identical:
            print(f"✅ CLS embeddings are functionally identical at tolerance {tolerance}")
    ...
    for i in range(num_images):
        ...
        print(f"Image {i}: All embeddings max diff = {diff_all:.2e}, CLS max diff = {diff_cls:.2e}")
...
def main():
    ...
    all_emb_wrapper, cls_emb_wrapper = generate_all_embeddings_wrapper(stack, lat, lon, bounds, epsg_str, gsd)
    
    # Comprehensive comparison
    results = compare_embeddings_comprehensive(
        all_emb_core, cls_emb_core, all_emb_wrapper, cls_emb_wrapper, 
        "Core Clay", "GeoAI Wrapper"
    )
    ...
    correct_core, total_core, pred_core = test_downstream_performance_cls(cls_emb_core, "Core Clay")
    correct_wrapper, total_wrapper, pred_wrapper = test_downstream_performance_cls(cls_emb_wrapper, "GeoAI Wrapper")
    ...
    if all_identical and cls_identical and predictions_match:
        ...
        if not all_identical:
            ...
            print("   This is expected due to processing order and doesn't affect practical use")
    ...
    print(f"\nKey Metrics Summary:")
    print(f"All embeddings - Max diff: {results['all_embeddings']['max_diff']:.2e}, Correlation: {results['all_embeddings']['correlation']:.8f}")
    print(f"CLS embeddings - Max diff: {results['cls_embeddings']['max_diff']:.2e}, Correlation: {results['cls_embeddings']['correlation']:.8f}")
