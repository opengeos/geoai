...
def main():
    print("Simple allclose test between Clay and GeoAI wrapper")
    ...
    embeddings_wrapper = generate_embeddings_wrapper(stack, lat, lon, bounds, epsg_str, gsd)
    
    print(f"\nTesting allclose at different thresholds:")
    
    # Test numpy allclose
    print("Numpy allclose:")
    print(f"  1e-3: {np.allclose(embeddings_core, embeddings_wrapper, atol=1e-3)}")
    print(f"  1e-4: {np.allclose(embeddings_core, embeddings_wrapper, atol=1e-4)}")
    print(f"  1e-5: {np.allclose(embeddings_core, embeddings_wrapper, atol=1e-5)}")
    
    # Test torch allclose
    core_tensor = torch.from_numpy(embeddings_core)
    wrapper_tensor = torch.from_numpy(embeddings_wrapper)
    
    print("Torch allclose:")
    print(f"  1e-3: {torch.allclose(core_tensor, wrapper_tensor, atol=1e-3)}")
    print(f"  1e-4: {torch.allclose(core_tensor, wrapper_tensor, atol=1e-4)}")
    print(f"  1e-5: {torch.allclose(core_tensor, wrapper_tensor, atol=1e-5)}")
