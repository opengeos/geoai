...
def setup_model():
    ...
    model = ClayMAEModule.load_from_checkpoint(
        ckpt, model_size="large", metadata_path="geoai/config/clay_metadata.yaml",
...
def method_individual(stack, lat, lon, model, device):
    ...
    for i, datetime_obj in enumerate(datetimes):
        ...
        pixels_single = transform(torch.from_numpy(stack[i].data.astype(np.float32))).unsqueeze(0)
        
        # Process single timestamp
        week_norm, hour_norm = normalize_timestamp(datetime_obj)
        time_tensor = torch.tensor(week_norm + hour_norm, dtype=torch.float32, device=device).unsqueeze(0)
        
        # Process single location
        lat_norm, lon_norm = normalize_latlon(lat, lon)
        latlon_tensor = torch.tensor(lat_norm + lon_norm, dtype=torch.float32, device=device).unsqueeze(0)
