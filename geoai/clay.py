...
class Clay:
    ...
    def _get_raster_bounds_wgs84(
        ...
        if src.crs != "EPSG:4326":
            ...
            transformed_xs, transformed_ys = transform_coords(
                src.crs, "EPSG:4326", xs, ys
            )
    ...
    def predict(
        self, tile_size: int = 256, overlap: float = 0.0, only_cls_token: bool = False
    ) -> Dict[str, Any]:
        """
    ...
    def _process_batch_images(
        self, sources: list, sensor_types: list, dates: list, gsd_overrides: list
        ...
        for i, (source, sensor_type, date, gsd_override) in enumerate(
            ...
            if save:
                ...
                output_path = os.path.join(
                    output_dir, f"{base_name}_embeddings.{format}"
                )
    ...
    def generate(
        ...
        if len(sources) == 1 or any(
            self._needs_tiling(src, tile_size) for src in sources
        ):
    ...
    def generate_from_dir(
        ...
        for i in range(0, len(image_files), batch_size):
                ...
                if metadata_dir:
                    ...
                    if os.path.exists(metadata_file):
                        try:
                            ...
                            print(
                                f"Warning: Could not load metadata for {img_file}: {e}"
                            )
            ...
            # Process batch
            print(
                f"Processing batch {i//batch_size + 1}/{(len(image_files) + batch_size - 1)//batch_size}"
            )
