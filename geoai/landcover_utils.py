...
def export_landcover_tiles(
    ...
    with rasterio.open(in_raster) as src:
                ...
                if file_ext in [".tif", ".tiff", ".img", ".jp2", ".png", ".bmp", ".gif"]:
                    try:
                        ...
                        if class_src.crs != src.crs:
                            if not quiet:
                                print(f"Warning: CRS mismatch between image ({src.crs}) and mask ({class_src.crs})")
        ...
        for tile_idx, (x, y) in enumerate(pbar):
            ...
            if not quiet:
                pbar.set_description(f"Exporting tiles ({stats['tiles_exported']}/{tile_idx + 1})")
    ...
    if not quiet:
        ...
        if min_feature_ratio is not False:
            print(f"Tiles skipped (low feature ratio < {min_feature_ratio}): {stats['tiles_skipped_ratio']}")
        print(f"\nOutput directories:")
