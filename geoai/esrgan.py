...
class ESRGANGenerator(nn.Module):
    ...
    def forward(self, input_tensor: torch.Tensor) -> torch.Tensor:
        ...
        base = F.interpolate(input_tensor, size=(hr_h, hr_w), mode='bicubic', align_corners=False)
...
class ESRGANDataPreprocess:
    ...
    def __init__(self,
        ...
        assert target_band_files is not None or target_file is not None, "Either target_band_files or target_file must be provided"
        assert (input_band_files is not None or input_file is not None) or use_downsampled_targets, "Either input_band_files or input_file must be provided"
    ...
    def preprocess_single_inputs(self,
                                 ds: gdal.Dataset,
                                 band: int,
                                 resample_method: LiteralString['bilinear', 
                                                                'cubic', 
                                                                'average', 
                                                                'cubicspline',
                                                                'max', 
        ...
        for i in range(no_tiles_width):
            for j in range(no_tiles_height):
                ...
                out_ds.SetGeoTransform((xmin + xoff * xres, xres, xskew, ymax + yoff * yres, yskew, yres))
    ...
    def preprocess_band(self, 
                        ...
                        band: int,
                        is_target: bool = True,
                        tile_ranges: List[Tuple[float, float, int, int, float, float]] = []) -> List[Tuple[float, float, int, int, float, float]]:
        """
        ...
        if not is_target:
            if sr != tile_ranges[0][6]:
                ...
                reproj = os.path.join(os.path.dirname(fname), f"reproj_{os.path.basename(fname)}")
        ...
        assert xres > 0 and yres < 0, "Geotransform must have positive xres and negative yres"
    ...
    def initiate_preprocessing(self):
        ...
        if self.target_file is not None:
            ...
            for band in self.target_band_files.keys():
                ...
                if not self.use_downsampled_targets:
                    these_tile_ranges = self.preprocess_band(ds, band, is_target=True, tile_ranges=[])
                else:
                    self.preprocess_single_inputs(ds, band)
        if self.input_file is not None:
            ...
            for band in range(1, ds.RasterCount + 1):
                ...
                these_tile_ranges = self.preprocess_band(ds, band, is_target=False, tile_ranges=these_tile_ranges)
    ...
    @classmethod
    def _match_nulls(cls, arr1:np.ndarray, arr2:np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        if arr2.shape != arr1.shape:
            ...
            dst_transform = Affine.translation(0, 0) * Affine.scale(src_w / dst_w, src_h / dst_h)
        ...
        arr1[mask] = 0
        arr2 = np.where(np.logical_or(arr2 <= 0, 
                            np.isnan(arr2)), 0, arr2)
        return arr1, arr2
    ...
    @classmethod
    def _get_as_array(cls, raster_file: str, shape: Tuple[int, int], band: int=1) -> np.ndarray:
        """
    ...
    def _noise(cls, arr, gamma=0.05):
        ...
        """
        local_noise = np.random.rand(arr.shape[0], arr.shape[1]) * gamma * arr.max()  # Keep scaled to input array values
        arr += local_noise
        return cls._normalize(arr, arr.min(), arr.max())

    @classmethod
    def _pad(cls, arr: np.ndarray, target_shape: Tuple[int, int], fill_value: float=-9999) -> np.ndarray:
        """
...
class ESRGAN:
    ...
    def train_esrgan(self,
        ...
        if not load_tensors and (low_res is None or high_res is None):
            raise ValueError("Either load_tensors must be True or both low_res and high_res must be provided.")
        elif load_tensors and not os.path.exists(os.path.join(self.data_path, "x_hr_tensors.pt")):
            raise ValueError("load_tensors is True but x_hr_tensors.pt not found in data_path.")
        elif load_tensors and not os.path.exists(os.path.join(self.data_path, "y_lr_tensors.pt")):
            raise ValueError("load_tensors is True but y_lr_tensors.pt not found in data_path.")
        ...
        gen = ESRGANGenerator(in_nc=input_channels, out_nc=output_channels, nf=nfilters, nb=nblocks, gc=ngroups, scale=self.scale).to(device)
        self.generator = gen
        ...
        scheduler_G = torch.optim.lr_scheduler.ReduceLROnPlateau(opt_gen,
                                                       mode='max',
                                                       factor=0.5,
                                                       patience=3,
                                                       threshold=1e-3,
        ...
        if n_samples is not None:
            ...
            val_loader = DataLoader(val_ds, batch_size=batch_size, sampler=sampler, 
                                    num_workers=4, pin_memory=True)
        else:
        ...
        if last_epoch > 0:
            ...
            if str(result) != '<All keys matched successfully>':
                raise ValueError('Mismatched weights on discriminator, check parameters.')
        ...
        for epoch in range(last_epoch, num_epochs):
            ...
            for (imgs_lr, imgs_hr) in tqdm(train_loader, ascii=True, total=batches):
                ...
                if adv_active:
                    ...
                    loss_G = (loss_pixel * lambda_pixel) + (loss_vgg * lambda_vgg) + (loss_G_adv * lambda_discriminator)
                else:
                ...
                if adv_active:
                    ...
                    fake = torch.full(pred_fake.shape, 0.).to(device)
            ...
            with torch.no_grad():
                for imgs_lr, imgs_hr in val_loader:
                    ...
                    # perceptual/VGG loss 
                    vgg_loss = criterion_perceptual(norm_to_vgg(sr), norm_to_vgg(imgs_hr)).item()
            ...
            print(f"Validation — PSNR: {mean_val_psnr:.3f}, VGG: {mean_val_vgg:.6f}, MSE: {mean_val_mse:.6f}")
            if mean_val_psnr > best_val_psnr:
                best_val_psnr = mean_val_psnr
                torch.save(gen.state_dict(), os.path.join(model_path, "best_generator.pt"))
                torch.save(dis.state_dict(), os.path.join(model_path, "best_discriminator.pt"))
            else:
            ...
            if non_improvement_counter >= 2:  # 2 consecutive epoch(s) without improvement
                non_improvement_counter = 0
                scheduler_D.step()
            ...
            torch.save(gen.state_dict(), os.path.join(self.model_path, "generator_checkpoint.pt"))
            torch.save(dis.state_dict(), os.path.join(self.model_path, "discriminator_checkpoint.pt"))
